...
def initialize_complex(
    ...
    original_skeleton_name : str
        name of the skeleton molecule
    source_data : str
        path to skeleton molecule
    substituent_name : str
        name of the substituent molecule
    path_to_database : str
        path to database of substituents
    path_to_skeletons : str
        path to skeletons
    path_to_substituents : str
        path to substituents
...
def main(
    ...
    # else:
    #     logger.log(logging.INFO, f"ChemSpaX working directory is set in environPment variable to {working_directory}")
    logger.log(
        logging.INFO, f"ChemSpaX working directory is set to {working_directory}"
    )

    # start ChemSpaX main loop to attach substituents to the skeleton
    logger.log(
        logging.INFO,
        "Looks like you want to add some substituents to a metal-ligand complex & optimize 'em.\n",
    )
    ...
    if not skeleton_list:
        ...
        logger.log(
            logging.INFO, f"{len(skeleton_list)} skeletons provided to functionalize"
        )
    ...
    all_substituents = [
        os.path.basename(os.path.normpath(substituent)).split(".")[0]
        for substituent in all_substituents
    ]
    ...
    for skeleton in skeleton_list:
        ...
        if substituent_list is not None:
            logger.log(
                logging.INFO,
                f"initializing complex by placing {substituent_list[0]} on {skeleton_name}",
            )
        else:
            logger.log(
                logging.INFO,
                f"initializing complex by placing random substituents based on length of functionalization_list",
            )
            ...
            substituent_list = random.choices(
                all_substituents, k=len_functionalization_list
            )
            logger.log(
                logging.INFO,
                f"initializing complex by placing {substituent_list[0]} on {skeleton_name}",
        ...
        if len(complex.functionalization_site_list) == 0:
            logger.log(
                logging.INFO, f"No functionalization sites found for {skeleton_name}"
            )
            continue
        ...
        if len(substituent_list) > len(complex.functionalization_site_list):
            logger.log(
                logging.INFO,
                f"Substituent list is larger than amount of specified functionalization sites, "
                f"only using first {len(complex.functionalization_site_list)} elements",
            )
            substituent_list = substituent_list[
                : len(complex.functionalization_site_list)
            ]
        # if substituent_list is too small, add random substituents to the list
        if len(substituent_list) < len(complex.functionalization_site_list):
            logger.log(
                logging.INFO,
                f"Substituent list is smaller than amount of specified functionalization sites, "
                f"adding random carbon (C) substituents to the list",
            )
            # add random substituents to the list
            for i in range(
                len(substituent_list), len(complex.functionalization_site_list)
            ):
        ...
        complex.generate_substituent_and_write_xyz(
            target_filename=first_target_file,
        ...
        copy_functionalization_list_xyz_2_mol(
            os.path.join(path_to_output, first_target_file + ".xyz"),
        ...
        for idx, substituent in enumerate(substituent_list[1:]):
            logger.log(
                logging.INFO,
                f"Attaching substituent: {substituent} iteration: {idx + 1}",
            )
            ...
            copy_functionalization_list_xyz_2_mol(
                os.path.join(path_to_output, target_name + ".xyz"),
                os.path.join(path_to_output, target_name + ".mol"),
            )
            logger.log(
                logging.INFO,
                f"Attached substituent: {substituent} iteration: {idx + 1}",
            )
...
if __name__ == "__main__":
    ...
    path_to_substituents = os.path.join(
        current_directory, "substituents_xyz", "manually_generated/"
    )
    path_to_database = os.path.join(
        path_to_substituents, "central_atom_centroid_database.csv"
    )
    path_to_output = os.path.join(
        current_directory, "substituents_xyz", "automatically_generated"
    )
    path_to_skeletons = os.path.join(current_directory, "skeletons")

    # parse input arguments
    parser = argparse.ArgumentParser(
        prog="chemspax", description="Attach substituents to a skeleton molecule"
    )
    parser.add_argument(
        "-s",
        "--substituent",
        help="Name of substituent to attach",
        required=False,
        action="append",
        default=None,
    )
    parser.add_argument(
        "-k",
        "--skeleton",
        help="Provide folder where skeletons are located",
        required=False,
        action="store",
        default=None,
    )
    ...
    parser.add_argument(
        "-pd",
        "--prepare",
        help="do xyz and mol conversions, and add new substituents to substituent database",
        required=False,
        action="store_true",
        default=False,
    )
    ...
    if args.skeleton is not None:
        ...
        list_of_skeleton_files = glob.glob(
            os.path.join(chemspax_working_directory, "skeletons", "*.xyz")
        )
    ...
    if args.prepare:
        prepare_data(
            ...
            path_to_skeletons=path_to_skeletons,
        ...
        main(
            ...
            path_to_substituents=path_to_substituents,
            path_to_skeletons=path_to_skeletons,
            working_directory=chemspax_working_directory,
            path_to_output=path_to_output,
        )
